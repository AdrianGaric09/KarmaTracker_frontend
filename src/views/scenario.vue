<template>
  ...
  <div :style="cardStyle">
    ...
    <!-- Footer logo (cloned style from proceed.vue) -->
...
export default {
  ...
  gap: '1.5rem',
    ...
    menuIconStyle() {
      return { width: '84px', height: '84px', objectFit: 'contain', marginTop: '2.5rem' }
    },
    cardStyle() {
      return {
  ...
  marginTop: '2rem',
    ...
    dividerStyle() {
      return { height: '0', borderTop: '5px dotted #4a4a4a', margin: '0.25rem 1rem 0.5rem' }
    },
    optionSubtextStyle() {
      return { fontSize: '1rem', color: '#6b6b6b', textAlign: 'center', fontStyle: 'italic', padding: '0 0.5rem' }
    },
    orStyle() {
      return { textAlign: 'center', fontSize: '1.2rem', color: '#4a4a4a', fontWeight: 600, margin: '0.1rem 0' }
    },
    buttonsRowStyle() {
      return { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '2.25rem', marginTop: '0.5rem' }
    },
    choiceBtnStyle() {
      return {
        width: '72px',
  ...
  methods: {
    onChoose(direction) {
      // First click on any arrow shows the popup; after acknowledging, proceed as needed
...
/* 1:1 popup animation from yes.vue */
.popup-enter-active {

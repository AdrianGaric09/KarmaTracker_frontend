...
export default {
  ...
  email: '',
      hovered: '',
      exitHover: false,
      bgSrc: require('@/assets/welcome_user.jpg'),
      orbitAngle: 0,
      orbitActive: false,
      orbitRadius: 18,
    }
  ...
  if (user && user.email) this.email = user.email;
    ...
    } catch(e) { console.debug('No stored user available', e); }
    setTimeout(() => {
  ...
  computed: {
    displayedUsername() {
      ...
      if (this.email && this.email.includes('@')) return this.email.split('@')[0];
      return '';
    },
    welcomeBgStyle() {
      return {
    ...
    questionOrbitStyle() {
      ...
      const x = Math.cos(angleRad) * this.orbitRadius;
      const y = Math.sin(angleRad) * this.orbitRadius;
      return {
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center',
    ...
    buttonBaseStyle() {
      return {
        ...
        cursor: 'pointer',
        transition: 'all 0.8s',
        textShadow: '0 0 10px #fff, 0 0 2px #FFD400',
        outline: 'none'
      }
    },
    yesButtonStyle() {
      return this.hovered === 'yes'
        ? {

    ...
    <div :style="proceedContainerStyle">
      <div :style="textContainerStyle">
        <div :style="mainTextStyle">
          ...
          section!<br>
          <br>
          Browse through to see<br>
          what each choice has<br>
          brought you.
        </div>
      </div>
    ...
    try {
      ...
      if (userData) {
        ...
        if (user && user.email) {
          ...
          const hasProceeded = localStorage.getItem(`kt_mi_proceed_${safeEmail}`);
    ...
    handleProceed() {
      try {
        ...
        if (userData) {
          ...
          if (user && user.email) {
            const safeEmail = user.email.toLowerCase().replace(/[^a-z0-9@._-]/g, '');
            localStorage.setItem(`kt_mi_proceed_${safeEmail}`, 'true');
          }
        }
      } catch (error) {
